...
ggplot(df, aes(x=category, y=sentiment, label=name, group=category)) +
  stat_summary(fun = mean, geom = "bar", fill=palette4, color=palette4, alpha=.1, lwd=.5) +
  geom_abline(intercept=0, slope=0, size=0.25) +
  ylab('Average sentiment') +
  xlab(NULL) +
  stat_summary(fun.data = mean_se, geom = "errorbar", width=0, lwd=0.5, color=palette4)
  #stat_summary(fun.data = mean_se, geom = "errorbar", width=0, lwd=1, color="white", position=position_nudge(x=.02)) +
  #stat_summary(fun.data = mean_se, geom = "errorbar", width=0, lwd=1, color=palette4, position=position_nudge(x=-.02))
